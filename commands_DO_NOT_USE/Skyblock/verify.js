...
module.exports = {
	...
	execute(client, message, args) {
		...
		const verifyrole = message.guild.roles.cache.find(role => role.name === 'Linked');
				...
				axios.get(`https://api.hypixel.net/player?key=${config.apikey}&uuid=${uuid}`).then(res => {
					if (res.data.player.socialMedia.links.DISCORD != message.author.tag) {
						...
						axios
							...
							.then(res => {
								const { level } = res.data.data.dungeons.types.catacombs;
								const round = level.toFixed();
								...
								const vembed = new Discord.MessageEmbed()
									...
									.setColor('GREEN');
								message.channel.send({ embeds: [vembed] }).then(msg => setTimeout(() => {
									msg.delete();
								}, 10000));
							});
					}
