  ...
  const config = require('../../Constants/Bot/config.json');
    ...
    if (inventory.prices.length == 0) {
      ...
      while (i < inventory.prices.length) {
        ...
        if (inventory.prices[i].item.name == undefined) {
          inventorytext += `[Lvl ${inventory.prices[i].value.pet_level}] ${caps(inventory.prices[i].item.tier)} ${caps(inventory.prices[i].item.type)} `
        }
    ...
    if (ec.prices.length == 0) {
      ...
      while (i < ec.prices.length) {
        ...
        if (ec.prices[i].item.name == undefined) {
          ectext += `[Lvl ${ec.prices[i].value.pet_level}] ${caps(ec.prices[i].item.tier)} ${caps(ec.prices[i].item.type)} `
        }
    ...
    if (storage.prices.length == 0) {
      ...
      while (i < storage.prices.length) {
        ...
        if (storage.prices[i].item.name == undefined) {
          storagetext += `[Lvl ${storage.prices[i].value.pet_level}] ${caps(storage.prices[i].item.tier)} ${caps(storage.prices[i].item.type)} `
        }
    ...
    if (pets.prices.length == 0) {
      ...
      while (i < pets.prices.length) {
        ...
        if (pets.prices[i].item.type != undefined) {
          petstext += `[Lvl ${pets.prices[i].value.pet_level}] ${caps(pets.prices[i].item.tier)} ${caps(pets.prices[i].item.type)} `
        }
