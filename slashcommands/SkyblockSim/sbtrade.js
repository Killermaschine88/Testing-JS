...
module.exports = {
  ...
  async execute(interaction, mclient) {
    ...
    if(trades != null && (action == 'send-offer' || action == 'reply-offer')) {
      ...
      .setDescription(`You already have an ongoing trade to <@!${trades.reciever.id}> (${trades.reciever.id}) ask them to accept or deny it so you can send trades again`)
      .setColor('RED')
    ...
    if(finditem == undefined || finditem.amount == 0) {
      ...
      .setDescription(`Couldn\'t find any Items matching \`${caps(tradeitem)}\` or Amount being above 0, make sure you spelled it corrrectly.`)
      .setFooter('Skyblock Simulator')
      .setColor('RED')
    ...
    if(amount > finditem.amount) {
      ...
      await collection1.updateOne(
          ...
          { $set: { sender: {
            id: interaction.user.id, item: caps(tradeitem), amount: amount, accepted: false
          }, reciever: {
      ...
      await collection.updateOne(
          { _id: interaction.user.id, "data.inventory.items.name": caps(tradeitem) },
    ...
    await collection1.updateOne(
          { _id: user.id },
          { $set: { reciever: {
            id: interaction.user.id, item: caps(tradeitem), amount: amount, accepted: true
          } } },
          { upsert: true })
      ...
      let user2 = await await collection.findOne({ _id: offer.sender.id })
      console.log(offer)
