...
module.exports = {
	...
	async execute(interaction, mclient) {
		...
		if (action == 'send-offer') {
			...
			const finditem = player.data.inventory.items.find(item => item.name == caps(tradeitem));
			...
			const user1 = await collection.findOne({ _id: offer.reciever.id });
			const user2 = await await collection.findOne({
				_id: offer.sender.id,
			});
			...
			try {
				...
				const fetched = await interaction.client.users.fetch(offer.sender.id);
				await fetched.send({ embeds: [tradereq] });
			} catch (e) {}
			try {
				const tradereq = new Discord.MessageEmbed()
				...
				const fetched = await interaction.client.users.fetch(offer.reciever.id);
				await fetched.send({ embeds: [tradereq] });
			} catch (e) {}
