...
module.exports = {
	...
	async execute(interaction, mclient) {
		...
		let rodname = '',
		 ...
		 swordinv = player.data.inventory.sword,
		 armorinv = player.data.inventory.armor,
		 swordcost = '',
		 armorcost = '',
		...
		const cancel_button = new Discord.MessageButton().setCustomId('cancel').setLabel('Cancel')
			.setStyle('DANGER');
		...
		await menu
			...
			.then(async i => {
				...
				if (id == 'rod') {
					...
					const expirationtime = Math.floor(Date.now() / 1000) + 172800;
